...
class DynamoDbDriver {
    ...
    //inde
    // async find(model, { query, search, perPage, index , ...rest}) {
    // TODO make like model.getIndexes()
    // async find({ model, options }) {
    //     const clonedOptions = { limit: 10, offset: 0, ...options };
    async find({ query, search, perPage, index , ...rest}) {
        ...
        // const queryResult = this._convertQueryToParams(query);
        // const searchResult = this._convertSearchToParams(search);
        ...
        const result = this._mergeExpressions([
            queryResult,
            searchResult,
        ]);

        const filterExpression = this._$and(result.filterExpressionArray.filter(item => !!item));
  ...
  //     return db.query(scanParams).promise();
    ...
    _convertSearchToParams(search) {
        ...
        const expressions = fields.map(key => this._buildQuery({
            operator: 'contains',
            fieldName: key,
            value: query,
        }));
    ...
    _mergeExpressions(expressions) {
        return expressions.reduce((obj, expr) => ({
...
async function runTest() {
    ...
    const found = await testDB.find({
        page: 3,
        // perPage: 1,
        // INDEX = KeyConditional
        // look at hoc with indexes(like withIndexes) iterate throught it
        index: {
            indexName: 'name-index',
            // TODO add check for empty expression
            query: {
                // name: 'Test 3',
            },
        },
        ...
        sort: { createdOn: -1, id: 1 }, // works only with query 
        // next key 
        // TO do pagination I need to get all items and make paging on my side(worst option) 
        // TO I can get all ids and make paging on my side
        // Dynamo has no skip and offset etc 
        // DynamoDb politics says pagination is not good for dynamo - if you want make your own

        // dynamodb returns nextKey
        // QUERY = FilterExpression
        search: {
            query: "world",
            fields: ["hello", "id"],
